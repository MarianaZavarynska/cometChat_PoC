...
import { INCOMING_MESSAGE_SOUND } from "../messages/resources/incomingMsgSound";
...
export class ConversationListComponent  implements OnInit, OnChanges {
  ...
  markConversationAsDelivered(conversation:CometChat.Conversation){
    ...
    if(conversation.getConversationType() == CometChat.RECEIVER_TYPE.USER && message  && !message.getDeliveredAt() &&  this.loggedInUser && (this.loggedInUser as any).uid != message.getSender().getUid()){
      CometChat.markAsDelivered(message as any)
    }
  }
  /**
    ...
    try {
      CometChat.getLoggedinUser()
        .then((user: any) => {
          ...
          this.fetchNextConversation()
            ...
            .catch((error: any) => {
              ...
              logger(
                "[CometChatConversationList] getConversations fetchNext error" as any,
                error
              );
            });
        })
        .catch((error) => {
          this.decoratorMessage = COMETCHAT_CONSTANTS.ERROR;
          logger(
            "[CometChatConversationList] getConversations getLoggedInUser error" as any,
            error
          );
        });
    } catch (error) {
  ...
  updateConversation(message: object, notification = true) {
    try {
      ...
      this.makeConversation(message)
        ...
        .catch((error) => {
          logger(
            "This is an error in converting message to conversation" as any,
            error
          );
        });
    } catch (error) {
