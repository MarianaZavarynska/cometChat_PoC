...
@Component({
  selector: 'app-conversation-list-with-messages',
  templateUrl: './conversation-list-with-messages.component.html',
  styleUrls: ['./conversation-list-with-messages.component.scss'],
  animations: [
...
export class ConversationListWithMessagesComponent  implements OnInit {
  curentItem: any;
  lastMessage: any;
  item: any = null;
  type: string = "";
  ...
  ngOnInit() {
    try {
      ...
      CometChat.getLoggedinUser()
        ...
        .catch((error) => {
          logger("[CometChatUnified] getLoggedInUser error" as any, error);
        });
    } catch (error) {
      logger(error);
    }
    ...
    try {
      ...
      switch (action.type) {
        ...
        case enums.CALL_ERROR: {
          logger(
            "User List screen --> call couldn't complete due to error" as any,
            action.payLoad
          );
          break;
        }
  ...
  //       })
  //       .catch((error) => {
  //         logger("Blocking user fails with error" as any, error);
  //       });
  //   } catch (error) {
  //     logger(error);
  //   }
  ...
  //       })
  //       .catch((error) => {
  //         logger("unblocking user fails with error" as any, error);
  //       });
  //   } catch (error) {
  //     logger(error);
  //   }
  ...
  //     //   .catch((error) => {
  //     //
  //     //     logger("Call initialization failed with exception:" as any, error);
  //     //   });
  //   } catch (error) {
  //     logger(error);
  //   }
  ...
  acceptIncomingCall(call: any) {
    try {
      ...
      CometChat.getConversation(id, type)
        ...
        .catch((error) => {
          logger("error while fetching a conversation" as any, error);
        });
    } catch (error) {
      logger(error);
    }
